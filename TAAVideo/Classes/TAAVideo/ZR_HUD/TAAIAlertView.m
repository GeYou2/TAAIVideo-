...
@interface TAAIAlertView ()

@property (nonatomic, strong) UIView *bgView;

@end

@implementation TAAIAlertView
...
- (void)showInView:(UIView *)view {
    ...
    switch (self.type) {
        case TAAIAlertViewTypeNormal:
            self.showAnimation = [self p_defaultAlertShowAnimation];
            break;
        case TAAIAlertViewTypeActionSheet:
...
- (void)setType:(TAAIAlertViewType)type{
    _type = type;
    if (self.contentView && _type == TAAIAlertViewTypeActionSheet) {
    ...
    } else if(self.contentView && _type == TAAIAlertViewTypeNormal) {
...
- (void)setContentView:(UIView *)contentView{
    ...
    if (self.type == TAAIAlertViewTypeActionSheet) {
        _contentView.frame = CGRectMake((CGRectGetMaxX(self.frame) - CGRectGetWidth(_contentView.frame)) * 0.5, CGRectGetMaxY(self.frame), CGRectGetWidth(_contentView.frame), CGRectGetHeight(_contentView.frame));
